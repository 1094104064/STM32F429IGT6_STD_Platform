...
static int ops_st7735_init(void)
{
    static const st7735_handle_t st7735_handle = {
        ...
        .pf_spi_init                = bsp_linker_st7735_spi_init,
        .pf_spi_transmit_8bit       = bsp_linker_st7735_spi_transmit_8bit,
        .pf_spi_transmit_16bit      = bsp_linker_st7735_spi_transmit_16bit,
        .pf_spi_dma_transmit_8bit   = bsp_linker_st7735_spi_dma_transmit_8bit,
        .pf_spi_dma_transmit_16bit  = bsp_linker_st7735_spi_dma_transmit_16bit,
...
{
    gs_st7735_driver.pf_backlight_on(&gs_st7735_driver);
}

static void ops_st7735_backlight_off(void)
{
    gs_st7735_driver.pf_backlight_off(&gs_st7735_driver);
}

static void ops_st7735_backlight_set(uint8_t brightness)
{
    gs_st7735_driver.pf_backlight_set(&gs_st7735_driver, brightness);
}

static void ops_st7735_put_pixel(uint16_t x, uint16_t y, uint32_t color)
{
    gs_st7735_driver.pf_put_pixel(&gs_st7735_driver,x ,y, color);
}

static void ops_st7735_fill_area(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint32_t color)
{
    gs_st7735_driver.pf_fill_area(&gs_st7735_driver, x, y, width, height, color);
}

static void ops_st7735_fill_screen(uint32_t color)
{
    gs_st7735_driver.pf_fill_screen(&gs_st7735_driver, color);
}

static void ops_st7735_flush(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint32_t * data)
{
    gs_st7735_driver.pf_flush(&gs_st7735_driver, x, y, width, height, (void *)data);
}
...
static const display_ops_t st7735_ops = {
    ...
    .pf_fill_area           = ops_st7735_fill_area,
    .pf_fill_screen         = ops_st7735_fill_screen,
    .pf_flush               = ops_st7735_flush,
